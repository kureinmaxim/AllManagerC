...
def _config_path(app_data_dir: str) -> str:
    return os.path.join(app_data_dir or '.', 'config.json')
...
def load_ui_preferences(app_data_dir: str) -> Dict[str, str]:
    """Load language and zoom from the user profile, with safe defaults."""
    prefs = {
        'language': DEFAULT_LANGUAGE,
        'zoom': DEFAULT_ZOOM,
    }
    stored = _read_config(_config_path(app_data_dir)).get(UI_CONFIG_KEY)
...
def save_ui_preferences(
    ...
    *,
    ...
    current = load_ui_preferences(app_data_dir)
